...
import { ModuleMetadata } from "./module/metadata.ts";

/**
 * AsteroidReflect Implements static functions for Reflect library
 * @internal
 */
export class AsteroidReflect {
   ...
   * 
   * @param value - A value that contains attached ModuleMetadata
   * @param target - The target object on which to define ModuleMetadata
   * 
   * @internal
   */
  static setOwnModuleMetadata(value: ModuleMetadata, target: Function): void {
    Reflect.defineMetadata(this.MODULE_METADATA, value, target);
  }

  /**
   * getOwnModuleMetadata Gets the ModuleMetadata on the target object
   * 
   * @param target - The target object on which the ModuleMetadata is defined
   * 
   * @returns The ModuleMetadata value from the target if found; otherwise undefined
   * 
   * @internal
   */
  static getOwnModuleMetadata(target: Function): ModuleMetadata | undefined {
    return Reflect.getOwnMetadata(this.MODULE_METADATA, target);
  }

  /**
   * getDefaultModuleMetadata Gets the default ModuleMetadata value
   * 
   * @returns An empty ModuleMetadata value
   * 
   * @internal
   */
